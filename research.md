...
<section class="page-section shell"><p class="label">Research themes</p><div><h2>Areas of inquiry</h2><div class="theme-list">{% for item in site.data.research %}<article><span>{{ item.number }}</span><div><h3>{{ item.title | escape }}</h3><p>{{ item.summary }}</p></div></article>{% endfor %}</div></div></section>
